...
class MockLegalExtractionAdapter(LegalExtractionAdapter):
    async def extract_claims(self, text: str) -> list[dict]:
        ...
        # Dummy response
        return [
            {"description": "Mock claim 1 extracted from text.", "confidence": 0.95},
            {"description": "Mock claim 2 extracted from text.", "confidence": 0.88},
        ]
    ...
    async def extract_parties(self, text: str) -> list[dict]:
        ...
        # Dummy response
        return [
            {"name": "John Doe", "role": "PLAINTIFF", "type": "PERSON"},
            {"name": "Acme Corp", "role": "DEFENDANT", "type": "ORGANIZATION"},
        ]
    ...
    async def extract_evidence(self, text: str) -> list[dict]:
        ...
        return [{"description": "Mock Exhibit A", "relevance": "High"}]
...
def get_extraction_adapter() -> LegalExtractionAdapter:
    ...
    if adapter_type == "mock":
        if settings.is_secure_environment:
            raise RuntimeError(
                "CRITICAL: MockLegalExtractionAdapter is strictly prohibited in production."
            )
