...
async def check_idempotency(db: AsyncSession, key: str):
    ...
    if idem:
        if idem.status == "in_progress":
            raise ProblemException(status=409, title="Conflict", detail="Request already in progress")
        return idem
...
async def complete_idempotency(db: AsyncSession, key: str, status_code: int, response_body: dict):
    stmt = select(IdempotencyKey).where(IdempotencyKey.key == key)
    result = await db.execute(stmt)
