...
class BasePolicy:
    ...
    async def _enforce_matter(
        ...
        if not matter_id:
            raise ProblemException(
                status=400, title="Bad Request", detail="Matter scope is required."
            )
        ...
        if required_scopes and member.access_scope not in required_scopes:
            raise ProblemException(
                status=403,
                title="Forbidden",
                detail=f"Requires one of scopes: {required_scopes}",
            )


class MatterAccessPolicy:
    @staticmethod
    def can_list(context: RequestContext):
        BasePolicy._enforce(
            context,
            {
                Role.FIRM_ADMIN,
            ...
            "list assigned matters",
        )

    @staticmethod
    async def can_read(context: RequestContext, db: AsyncSession, matter_id: str):
        BasePolicy._enforce(
            context,
            {
                Role.FIRM_ADMIN,
        ...
        await BasePolicy._enforce_matter(
            context, db, matter_id, ["read", "write", "admin"]
        )

    @staticmethod
    async def can_write(context: RequestContext, db: AsyncSession, matter_id: str):
        BasePolicy._enforce(
            context,
            {Role.FIRM_ADMIN, Role.ATTORNEY, Role.PARALEGAL},
    ...
    async def can_create(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY, Role.PARALEGAL}, "create matters"
        )

    @staticmethod
    async def can_manage_members(
        context: RequestContext, db: AsyncSession, matter_id: str
    ):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY}, "manage matter members"
        )
        await BasePolicy._enforce_matter(context, db, matter_id, ["admin"])

    @staticmethod
    async def can_close(context: RequestContext, db: AsyncSession, matter_id: str):
...
class DocumentAccessPolicy:
    @staticmethod
    async def can_upload(context: RequestContext, db: AsyncSession, matter_id: str):
        BasePolicy._enforce(
            context,
            {Role.FIRM_ADMIN, Role.ATTORNEY, Role.PARALEGAL},
            "upload documents",
...
class ReviewAccessPolicy:
    @staticmethod
    async def can_approve(context: RequestContext, db: AsyncSession, matter_id: str):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY}, "approve or reject AI reviews"
        )
        await BasePolicy._enforce_matter(context, db, matter_id, ["write", "admin"])
...
class ExportAccessPolicy:
    ...
    def can_export(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY, Role.PARALEGAL}, "export drafts"
        )
...
class DraftAccessPolicy:
    ...
    def can_approve_external(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY}, "approve drafts for external use"
        )
...
class DeadlineAccessPolicy:
    ...
    def can_verify(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY}, "verify and approve deadlines"
        )
...
class AdminAccessPolicy:
    ...
    def can_manage_firm(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN}, "manage firm membership and billing"
        )

    @staticmethod
    def can_audit(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.AUDITOR}, "access audit logs"
        )

    @staticmethod
    def can_rebuild_mesa(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.FIRM_ADMIN, Role.ATTORNEY}, "rebuild MESA index"
        )
...
class SupportAccessPolicy:
    ...
    def can_access(context: RequestContext):
        BasePolicy._enforce(
            context, {Role.SUPPORT_TEMPORARY}, "access via temporary support mode"
        )
