...
def set_tenant_id(tenant_id: str | None) -> contextvars.Token:
    """Set the request/job tenant and return a token that must be reset."""
    ...
    if execute_state.is_select or execute_state.is_update or execute_state.is_delete:
        ...
        def filter_tenant(cls):
            def get_tenant_or_raise():
                ...
                if tid is None:
                    raise RuntimeError(
                        "RLS Guard: Attempted to query tenant-specific data without an active tenant context"
                    )
                return tid

            return cls.tenant_id == bindparam(
                "current_tenant_id_bind", callable_=get_tenant_or_raise
            )
