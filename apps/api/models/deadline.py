...
class DeadlineRule(Base, AuditMixin, TenantAwareMixin):
    ...
    duration_unit: Mapped[str] = mapped_column(
        String, default="days", nullable=False
    )  # days, weeks, months
    calculation_method: Mapped[str] = mapped_column(
        String, default="calendar_days", nullable=False
    ...
    legal_source_id: Mapped[str] = mapped_column(
        ForeignKey("legal_sources.id"), nullable=True
    )
...
class DeadlineCandidate(Base, AuditMixin, TenantAwareMixin):
    ...
    rule_id: Mapped[str] = mapped_column(
        ForeignKey("deadline_rules.id"), nullable=False
    )
    ...
    status: Mapped[DeadlineState] = mapped_column(
        SQLEnum(DeadlineState, name="deadline_state", create_type=False),
        default=DeadlineState.POTENTIAL_DEADLINE,
        nullable=False,
    )
    confidence_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    timezone: Mapped[str] = mapped_column(
        String, default="Europe/Istanbul", nullable=False
    )
...
class ApprovedDeadline(Base, AuditMixin, TenantAwareMixin):
    ...
    deadline_candidate_id: Mapped[str] = mapped_column(
        ForeignKey("deadline_candidates.id"), nullable=False
    )
