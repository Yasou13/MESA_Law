...
@router.get(
    ...
    operation_id="listParsedDocuments",
)
...
async def list_parsed_documents(
    ...
    await MatterAccessPolicy.can_read(context, db, document.matter_id)
...
@router.get(
    "/{parsed_document_id}/pages",
    response_model=list[ParsedPageResponse],
    operation_id="listParsedPages",
)
@limiter.limit("60/minute")
async def list_parsed_pages(
    ...
    document = await db.get(Document, pdoc.document_id)
    if not document or document.tenant_id != context.tenant_id:
        raise HTTPException(status_code=404, detail="Parsed document not found")
    await MatterAccessPolicy.can_read(context, db, document.matter_id)
