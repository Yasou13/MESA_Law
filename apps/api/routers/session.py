...
class ActiveFirmResponse(BaseModel):
    status: str
    active_firm_id: str
    role: str
...
@router.get(
    "/context", response_model=SessionContextResponse, operation_id="getSessionContext"
)
async def get_session_context(context: RequestContext = Depends(setup_tenant_context)):
    return {
        "status": "success",
...
@router.post(
    "/active-firm", response_model=ActiveFirmResponse, operation_id="setActiveFirm"
)
async def set_active_firm(
    ...
    if not membership:
        raise HTTPException(
            status_code=403, detail="User is not an active member of the requested firm"
        )
