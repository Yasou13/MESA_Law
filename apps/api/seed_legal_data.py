...
async def seed_legal_data():
    ...
    async with AsyncSessionLocal() as db:
        ...
        async def create_document(matter, title, pages_texts):
            ...
            pdoc = ParsedDocument(document_id=doc.id, revision_id=rev.id, parser_used="mock", tenant_id=tenant_id)
            db.add(pdoc)
            await db.flush()
            
            for i, text_content in enumerate(pages_texts):
                page = ParsedPage(parsed_document_id=pdoc.id, page_number=i+1, text_content=text_content)
                db.add(page)
            await db.flush()
            return doc
        ...
        await create_document(matter_mevzuat, "6098 Türk Borçlar Kanunu", ["MADDE 1: Sözleşme, tarafların iradelerini karşılıklı ve birbirine uygun olarak açıklamalarıyla kurulur."])
        await create_document(matter_mevzuat, "6098 Türk Borçlar Kanunu (Eski Sürüm)", ["MADDE 1: (Mülga Metin) Akit, iki tarafın karşılıklı rızasıyla teşekkül eder."])
        await create_document(matter_mevzuat, "4721 Türk Medeni Kanunu", ["MADDE 1: Kanun, sözüyle ve özüyle değindiği bütün konularda uygulanır."])
        await create_document(matter_mevzuat, "4721 Türk Medeni Kanunu (Eski Sürüm)", ["MADDE 1: (Mülga) Kanun lafzıyla ve ruhuyla temas ettiği bütün meselelerde meridir."])
        
        # 20 Yüksek Mahkeme Kararı (spread across matters 1 and 2)
        for i in range(20):
            matter_ref = matters[1] if i < 10 else matters[2]
            title = f"Yargıtay {i+1}. Hukuk Dairesi E: 2023/{i*10} K: 2023/{i*10+5}"
