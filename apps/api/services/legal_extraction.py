...
class HeuristicLegalExtractionAdapter(LegalExtractionAdapter):
    ...
    async def extract_parties(self, text: str) -> list[dict]:
        ...
        def add_party(
            ...
            locator: str = "Regex pattern match",
        ):
            clean_name = name.strip(" .,\t\n\r\"'[]()")
            if (
                len(clean_name) < 2
                or len(clean_name) > 100
                or clean_name.lower() in seen_names
            ):
                return
            ...
            party_type = (
                "ORGANIZATION"
                if any(kw in clean_name.lower() for kw in org_keywords)
                else "PERSON"
            )
            parties.append(
                {
        ...
        v_match = re.search(
            r"([A-ZÇĞİÖŞÜ][A-Za-zçğıöşü0-9\s,._&'-]{2,50})\s+(?:v\.|vs\.|versus|-)\s+([A-ZÇĞİÖŞÜ][A-Za-zçğıöşü0-9\s,._&'-]{2,50})",
            text,
        )
        if v_match:
        ...
        p_match = re.search(
            r"(?i)(?:plaintiff|claimant|applicant|davacı|alacaklı|işçi|müşteki|başvuran)[:\s]+([A-ZÇĞİÖŞÜa-zçğıöşü0-9\s,._&'-]{2,50})",
            text,
        )
        if p_match:
        ...
        d_match = re.search(
            r"(?i)(?:defendant|respondent|davalı|borçlu|işveren|şüpheli|sanık)[:\s]+([A-ZÇĞİÖŞÜa-zçğıöşü0-9\s,._&'-]{2,50})",
            text,
        )
        if d_match:
    ...
    async def extract_events(self, text: str) -> list[dict]:
        ...
        trigger_patterns: list[dict[str, Any]] = [
            ...
            },
            {
                ...
                "rules": [
                    {
                        "rule_name": "İcranın Geri Bırakılması - 7 Gün",
                        "offset_days": 7,
                        "reference": "İİK 33",
                    },
                ],
        ...
        for tp in trigger_patterns:
            trigger_match = re.search(tp["pattern"], text, re.IGNORECASE)
            if trigger_match:
                    ...
                    events.append(
                        {
                            ...
                            "confidence": 0.85,
                            "evidence_text": trigger_match.group(0),
        ...
        for match in date_matches:
            for event in events:
                if "date" not in event:
                    event["detected_date"] = (
                        f"{match[2]}-{match[1].zfill(2)}-{match[0].zfill(2)}"
                    )
                    break
...
class LLMEnhancedExtractionAdapter(LegalExtractionAdapter):
    ...
    async def _check_llm(self):
        ...
        if self._llm_available is None:
            try:
                from apps.api.core.llm_client import LLMProvider, get_llm_client

                client = get_llm_client()
                self._llm_available = client.config.provider != LLMProvider.MOCK
            except Exception:  # noqa: BLE001 - optional LLM provider boundary
    ...
    async def _llm_extract(self, text: str, task: str) -> list[dict]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"LLM returned non-JSON for {task}, using heuristic only"
                )
                return []
        except Exception as e:  # noqa: BLE001 - optional LLM provider boundary
    ...
    async def extract_parties(self, text: str) -> list[dict]:
        ...
        llm_results = await self._llm_extract(
            text, "Tarafları (davacı, davalı, müdahil) çıkar"
        )
    ...
    async def extract_events(self, text: str) -> list[dict]:
        ...
        llm_results = await self._llm_extract(
            text, "Hukuki olayları ve süre tetikleyicilerini çıkar"
        )
