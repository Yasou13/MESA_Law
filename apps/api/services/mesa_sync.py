...
class MesaSyncService:
    ...
    async def sync_matter(self, db: AsyncSession, tenant_id: str, matter_id: str) -> int:
        """
        ...
        for parsed_doc, doc in rows:
            synced_pages += await self.sync_document(db, tenant_id, parsed_doc.id, matter_id, doc.title)
            
        logger.info(f"Matter {matter_id} sync complete. Sent {synced_pages} pages to MESA.")
        return synced_pages

    async def sync_document(self, db: AsyncSession, tenant_id: str, parsed_doc_id: str, matter_id: str, doc_title: str) -> int:
        """
