...
async def test_document_revision_model():
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            doc = Document(
                matter_id=matter.id, title="Initial Brief", tenant_id=tenant_id
            )
            session.add(doc)
            await session.flush()
            ...
            assert rev_fetched is not None
            assert rev_fetched.version == 1
            assert rev_fetched.s3_key == rev1_key
            assert rev_fetched.scan_status == DocumentState.READY
            assert rev_fetched.is_canonical is True
    finally:
