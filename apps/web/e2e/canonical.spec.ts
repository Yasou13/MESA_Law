...
test('canonical workflow: login -> matter -> upload -> view', async ({ page }) => {
  ...
  // file input triggers upload immediately onChange
  ...
  // Verify upload success and document presence in the list
  await expect(page.locator('text=test-doc.pdf')).toBeVisible({ timeout: 15000 });
});
