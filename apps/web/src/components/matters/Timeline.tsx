...
export function Timeline() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      // Simulate 10% chance of degraded error
      if (Math.random() > 0.9) {
        setError(true);
      } else {
        setLoading(false);
  ...
  if (error) {
    return (
      <div className="p-8 bg-red-900/20 border border-red-900 rounded-lg text-red-200">
        ...
        <button onClick={() => { setLoading(true); setError(false); }} className="mt-4 text-sm bg-red-800/50 hover:bg-red-800 px-4 py-2 rounded">Retry</button>
      </div>
    );
  }
