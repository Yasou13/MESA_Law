...
async def scan_with_clamav(stream) -> bool:
    ...
    try:
        ...
        return "OK" in response_text and "FOUND" not in response_text
...
async def handle_scan_document(payload: dict, session: AsyncSession):
    ...
    if not revision_id or not s3_key:
        raise TerminalJobError("Missing revision_id or s3_key in payload")
    ...
    async with storage_service.session.client(
        ...
        try:
            response = await s3.get_object(
                Bucket=storage_service.bucket_name, Key=s3_key
            )
            ...
            if rev:
                rev.scan_status = (
                    DocumentState.CLEAN if is_clean else DocumentState.INFECTED
                )
                ...
                if is_clean:
                    ...
                    job = Job(
                        ...
                        idempotency_key=f"parse:{revision_id}",
                        payload={
                            "document_id": doc_id,
                ...
                raise TerminalJobError(f"Revision {revision_id} not found in DB")
