...
async def handle_parse_document(payload: dict, session: AsyncSession):
    ...
    if not document_id or not revision_id or not s3_key:
        raise TerminalJobError("Missing required payload fields")
    ...
    try:
        async with storage_service.session.client(
            ...
            if HAS_LLAMA_PARSE and api_key and not is_docx:
                ...
                docx_text = await asyncio.to_thread(docx2txt.process, temp_path)
                parser_used_name = "docx2txt"
                pages = [{"page_number": 1, "text": docx_text, "layout": None}]
                parsed_text = docx_text
                ...
                pdf_bytes = await asyncio.to_thread(Path(temp_path).read_bytes)
                ...
                if settings.is_secure_environment:
                    raise RuntimeError(
                        "No parser available. Mock extraction is strictly prohibited in production."
                    )
                ...
                pages = [
                    {
                        "page_number": 1,
                        "text": "Mock parsed content for page 1.",
                        "layout": None,
                    }
                ]
            ...
            await session.execute(
                text(
                    "UPDATE draft_citations SET verification_state = 'STALE_REVISION' WHERE document_id = :doc_id AND document_revision_id != :new_rev AND verification_state != 'STALE_REVISION'"
                ),
