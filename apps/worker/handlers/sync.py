...
async def handle_sync_mesa_document(payload: dict, session: AsyncSession):
    ...
    if not parsed_document_id:
        raise TerminalJobError(
            "Missing parsed_document_id in SYNC_MESA_DOCUMENT payload"
        )

    from apps.api.core.factory import get_ingestion_adapter
...
async def handle_publish_review(payload: dict, session: AsyncSession):
    ...
    if not review_id:
        raise TerminalJobError("Missing review_id")

    review = await session.get(ReviewItem, review_id)
    if not review:
        raise TerminalJobError(f"Review {review_id} not found")

    if review.status != ReviewState.APPROVED_PENDING_PUBLICATION:
        raise TerminalJobError(
            f"Review {review_id} is not APPROVED_PENDING_PUBLICATION"
        )

    try:
        # Phase 10: Use corrected_content if available, otherwise proposed_content
        content = (
            review.corrected_content
            if review.corrected_content
            else (review.proposed_content or {})
        )
        ...
        if review.entity_type == "party":
            ...
            if not name or name == "Unknown Party":
                raise ValueError(
                    "PARTY_NAME_REQUIRED: Party name is required for canonical insert"
                )
            ...
            if (
                ...
                or not defendant_id
                or claimant_id
                in ("default_claimant", "unknown-party-id", "placeholder")
                or defendant_id
                in ("default_defendant", "unknown-party-id", "placeholder")
            ):
                raise ValueError(
                    "PARTY_LINK_REQUIRED: Claims must be linked to valid MatterParty IDs"
                )
            ...
            if not description:
                raise ValueError(
                    "EVIDENCE_DESCRIPTION_REQUIRED: Description is required"
                )
        ...
        if isinstance(e, ValueError):
            raise TerminalJobError(str(e)) from e
...
async def handle_build_lexical_index(payload: dict, session: AsyncSession):
    ...
    try:
        ...
        raise RetryableJobError(f"Lexical index maintenance failed: {e}") from e
