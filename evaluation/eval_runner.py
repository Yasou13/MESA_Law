...
def load_golden_dataset(path: Path | None = None) -> dict:
    ...
    logger.info(
        f"Loaded golden dataset: {len(entries)} entries, version {data.get('version', 'unknown')}"
    )
    ...
    for entry in entries:
        if "id" not in entry or "category" not in entry:
            raise ValueError(
                f"Invalid entry in golden dataset: missing id or category — {entry}"
            )
...
async def run_extraction_benchmark(entries: list[dict]) -> EvaluationReport:
    ...
    for entry in entries:
        ...
        if subcategory == "party_extraction":
            ...
            f1 = compute_entity_extraction_f1(
                predicted, expected_parties, match_key="name"
            )
            ...
            logger.info(
                f"  [{entry_id}] Party F1={f1:.2f}, Role Accuracy={role_acc:.2f}"
            )
            ...
            expected_for_match = [
                {"name": c.get("description_contains", "")} for c in expected_claims
            ]
            predicted_for_match = [
                {"name": p.get("description", "")} for p in predicted
            ]

            f1 = compute_entity_extraction_f1(
                predicted_for_match, expected_for_match, match_key="name"
            )
            claim_f1_scores.append(f1)
            logger.info(
                f"  [{entry_id}] Claim F1={f1:.2f} (predicted {len(predicted)}, expected {len(expected_claims)})"
            )
            ...
            expected_for_match = [
                {"name": e.get("description_contains", "")} for e in expected_evidence
            ]
            predicted_for_match = [
                {"name": p.get("description", "")} for p in predicted
            ]
            ...
            logger.info(
                f"  [{entry_id}] Event detection={'PASS' if detected else 'FAIL'} (predicted {len(predicted)}, expected {len(expected_events)})"
            )
    ...
    if claim_f1_scores:
        ...
        report.metrics.append(
            evaluate_gate("claim_extraction_f1", avg_claim_f1, threshold=0.60)
        )

    if evidence_f1_scores:
        avg_ev_f1 = sum(evidence_f1_scores) / len(evidence_f1_scores)
        report.metrics.append(
            evaluate_gate("evidence_extraction_f1", avg_ev_f1, threshold=0.60)
        )

    if event_detection_scores:
        avg_event = sum(event_detection_scores) / len(event_detection_scores)
        report.metrics.append(
            evaluate_gate("event_detection_accuracy", avg_event, threshold=0.50)
        )
...
async def run_deadline_benchmark(entries: list[dict]) -> EvaluationReport:
    ...
    for entry in entries:
        ...
        if match:
            ...
            logger.error(
                f"  [{entry_id}] FAIL: got {calculated_date}, expected {expected_date} ({expected.get('notes', '')})"
            )
...
async def run_qa_benchmark(entries: list[dict]) -> EvaluationReport:
        ...
        if subcategory in ("factual_retrieval", "multi_document_reasoning"):
            ...
            if must_not_contain:
                ...
                if contamination == 0.0:
                    ...
                    logger.info(
                        f"  [{entry_id}] Negative test PASS: no forbidden content in context"
                    )
                else:
                    answer_coverage_scores.append(0.0)
                    logger.error(
                        f"  [{entry_id}] Negative test FAIL: forbidden content found"
                    )
            ...
            contamination = compute_cross_matter_contamination_rate(
                matter_b_docs, must_not
            )
            contamination_scores.append(contamination)
            logger.info(
                f"  [{entry_id}] Cross-matter contamination={contamination:.2f}"
            )
            ...
            mock_citation = [
                {
                    "document_id": available_docs[0]["id"],
                    "page_number": expected.get("must_not_reference_page", 999),
                }
            ]
            fab_rate = compute_fabricated_citation_rate(mock_citation, available_docs)
            fabrication_scores.append(fab_rate)
            logger.info(
                f"  [{entry_id}] Fabrication rate={fab_rate:.2f} (expected >0 for this test case)"
            )
    ...
    if fabrication_scores:
        ...
        avg_fabrication_detection = sum(1.0 for f in fabrication_scores if f > 0) / len(
            fabrication_scores
        )
        report.metrics.append(
            evaluate_gate(
...
async def run_evaluation(
    ...
    if combined_report.all_passed:
        ...
        logger.error(
            f"🚫 {len(failures)} gate(s) FAILED. Model NOT approved for PILOT_CANARY."
        )
        for f in failures:
    ...
    if report_format == "json":
        ...
        report_json = json.dumps(
            combined_report.summary(), indent=2, ensure_ascii=False
        )
        await asyncio.to_thread(report_path.write_text, report_json, encoding="utf-8")
...
def main():
    ...
    parser.add_argument(
        "--category", type=str, default=None, help="Run specific category only"
    )
    parser.add_argument(
        "--report",
        type=str,
