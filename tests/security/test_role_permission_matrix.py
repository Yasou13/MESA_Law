...
MATRIX = [
    ...
    (
        "POST",
        "/api/v1/matters/mat_123/override-conflict",
        {"FIRM_ADMIN", "ATTORNEY"},
    ),
    (
        "POST",
        "/api/v1/documents/upload-intent",
        {"FIRM_ADMIN", "ATTORNEY", "PARALEGAL"},
    ),
    (
        "POST",
        "/api/v1/draft-studio/drafts/draft_123/approve",
        {"FIRM_ADMIN", "ATTORNEY"},
    ),
    ...
    def _override(role: str):
        async def mock_setup(request: Request):
            return RequestContext(
                tenant_id="firm_123", principal_id="user_123", roles={role}
            )
        ...
        async def mock_refresh(obj):
            if not getattr(obj, "id", None):
                obj.id = "mock_id"
            if (
                getattr(obj, "__tablename__", "") == "matters"
...
async def test_rbac_matrix(override_context, role, method, endpoint, allowed_roles):
    ...
    if role not in allowed_roles:
        assert res.status_code == 403, (
            f"Expected 403 for {role} on {method} {endpoint}, got {res.status_code}"
        )
    else:
        # If allowed, it shouldn't be 403 (might be 404/422/200 depending on mock DB)
        assert res.status_code != 403, (
            f"Expected NOT 403 for {role} on {method} {endpoint}, got 403"
        )
