...
def test_is_turkish_statutory_holiday():
    assert DeadlineEngine.is_turkish_statutory_holiday(datetime.date(2026, 1, 1)) is True
    assert DeadlineEngine.is_turkish_statutory_holiday(datetime.date(2026, 4, 23)) is True
    assert DeadlineEngine.is_turkish_statutory_holiday(datetime.date(2026, 8, 30)) is True
    # Ramazan Bayramı 2026: March 20-22
    assert DeadlineEngine.is_turkish_statutory_holiday(datetime.date(2026, 3, 21)) is True
    assert DeadlineEngine.is_turkish_statutory_holiday(datetime.date(2026, 3, 25)) is False
...
def test_calculate_date_adli_tatil_hmk_art_104():
    ...
    calc = DeadlineEngine.calculate_date(trigger, 10, business_days_only=False, jurisdiction="TR_HMK")
...
def test_calculate_date_iik_not_subject_to_adli_tatil():
    ...
    trigger = datetime.date(2026, 7, 15)
    calc = DeadlineEngine.calculate_date(trigger, 10, business_days_only=False, jurisdiction="TR_IIK")
...
def test_calculate_date_holiday_rollover():
    ...
    calc = DeadlineEngine.calculate_date(trigger, 2, business_days_only=False, jurisdiction="TR_IIK")
...
async def test_lawyer_review_stages():
    ...
    reviewed_pd = await DeadlineEngine.submit_for_review(mock_session, "pd_100", "firm_1", "Please check IIK rules")
    assert reviewed_pd is not None
    assert reviewed_pd.status == "under_review"
    assert "Please check IIK rules" in reviewed_pd.description
    
    # Stage 2: Approve
    approved = await DeadlineEngine.approve_deadline(mock_session, "pd_100", "firm_1", "lawyer_yasin")
    assert approved is not None
    assert approved.status == "approved" if hasattr(approved, 'status') else pd.status == "approved"
