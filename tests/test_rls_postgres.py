...
async def test_real_postgres_rls_isolation(db_engine):
    ...
    async with db_engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'mesa_law_app') THEN CREATE ROLE mesa_law_app NOLOGIN; END IF; END $$;"
            )
        )
        ...
        await conn.execute(
            text("ALTER TABLE test_real_rls_table ENABLE ROW LEVEL SECURITY;")
        )
        await conn.execute(
            text("ALTER TABLE test_real_rls_table FORCE ROW LEVEL SECURITY;")
        )
        ...
        await conn.execute(
            text(
                "GRANT USAGE, SELECT ON SEQUENCE test_real_rls_table_id_seq TO mesa_law_app;"
            )
        )

        # Insert data as superuser
        await conn.execute(
            text("SELECT set_config('app.current_tenant', 'tenant_A', true)")
        )
        await conn.execute(
            text(
                "INSERT INTO test_real_rls_table (tenant_id, data) VALUES ('tenant_A', 'A_data')"
            )
        )

        await conn.execute(
            text("SELECT set_config('app.current_tenant', 'tenant_B', true)")
        )
        await conn.execute(
            text(
                "INSERT INTO test_real_rls_table (tenant_id, data) VALUES ('tenant_B', 'B_data')"
            )
        )
